...
def simpleSearch(catModel):
    ...
    simpleResults_screen.geometry("650x620")
    ...
    Label(simpleResults_screen, text="Total Unsold: " + str(totalUnsoldCount)).pack()
    ...
    for i in unsoldGroup:
        ...
        x["PurchaseStatus"] = "Sold" 
        soldGroup = itemsCol.find(x)
        stockSold = soldGroup.count()
        ...
        if userID[0].upper() == "A":
            mylist.insert(END, str(x.get("Category")) + ", " + str(x.get("Model")) + ", " + str(x.get("Color")) + ", Price: " + str(x.get("Price")) + ", " + str(x.get("Warranty")) + " Months Warranty, Stock Avail: " + str(i.get("StockAvail")) + " Left" + ", Cost: " + str(x.get("Cost")) + ", Stock Sold: " + str(stockSold))
        else:
...
def advancedSearch(catModel, color, factory, prodYear, powerSupply):
    ...
    advancedResults_screen.geometry("650x620")
